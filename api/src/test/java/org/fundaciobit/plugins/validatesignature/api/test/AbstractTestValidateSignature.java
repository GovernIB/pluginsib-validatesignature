...
public abstract class AbstractTestValidateSignature implements SignatureConstants {
    ...
    public String internalCheckTestBasicResults(SignatureValidationTestResult result, String key,
        ...
        if (throwException) {
            ...
            if (!compareStr(result.getExpectedSignType(), response.getSignType())) {
            ...
            if (!compareStr(result.getExpectedSignProfile(), response.getSignProfile())) {
                return "Perfil de firma diferent en test " + key + "(Expected:" + result.getExpectedSignProfile()
                        + " | " + response.getSignProfile();
            }
    ...
    public Map<String, SignatureValidationTestResult> internalTestBasic(boolean checkResults) throws Exception {
        ...
        for (String key : tests.keySet()) {
            ...
            SignatureValidationTestResult result = new SignatureValidationTestResult(params[0], params[1], signature,
                    document, params[2], params[3], params[4], (params[5] == null ? false : true));
