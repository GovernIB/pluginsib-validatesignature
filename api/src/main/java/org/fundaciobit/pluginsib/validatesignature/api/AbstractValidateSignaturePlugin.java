...
public abstract class AbstractValidateSignaturePlugin extends AbstractPluginPropertiesTranslations
    ...
    public String filter(ValidateSignatureRequest vsr) {
        ...
        if (!checkRequiredSupported(required.getReturnSignatureTypeFormatProfile(),
            ...
            String msg = getTraduccio(VALIDATE_SIGNATURE_API_BUNDLE, "error.filter.typeformatprofile",
                    new Locale(vsr.getLanguage()));
            return msg;
        }
        if (!checkRequiredSupported(required.getValidateCertificateRevocation(),
                supported.getValidateCertificateRevocation())) {
            String msg = getTraduccio(VALIDATE_SIGNATURE_API_BUNDLE, "error.filter.certificaterevocation",
                    new Locale(vsr.getLanguage()));
            return msg;
        }
        if (!checkRequiredSupported(required.getReturnCertificateInfo(), supported.getReturnCertificateInfo())) {
            String msg = getTraduccio(VALIDATE_SIGNATURE_API_BUNDLE, "error.filter.certificateinfo",
                    new Locale(vsr.getLanguage()));
            return msg;
        }
        if (!checkRequiredSupported(required.getReturnValidationChecks(), supported.getReturnValidationChecks())) {
            String msg = getTraduccio(VALIDATE_SIGNATURE_API_BUNDLE, "error.filter.validationchecks",
                    new Locale(vsr.getLanguage()));
            return msg;
        }
        if (!checkRequiredSupported(required.getReturnCertificates(), supported.getReturnCertificates())) {
            String msg = getTraduccio(VALIDATE_SIGNATURE_API_BUNDLE, "error.filter.returncertificates",
                    new Locale(vsr.getLanguage()));
            return msg;
        }
        if (!checkRequiredSupported(required.getReturnTimeStampInfo(), supported.getReturnTimeStampInfo())) {
            String msg = getTraduccio(VALIDATE_SIGNATURE_API_BUNDLE, "error.filter.timestampinfo",
                    new Locale(vsr.getLanguage()));
            return msg;
        }
    ...
    public static void printSignatureInfo(ValidateSignatureResponse vs) {
        ...
        if (status.getStatus() == ValidationStatus.SIGNATURE_VALID) {
            ...
            System.out.println(
                    " status = " + ((status.getStatus() == ValidationStatus.SIGNATURE_INVALID) ? "INVALID" : "ERROR"));
        ...
        if (diList != null) {
            for (int i = 0; i < diList.length; i++) {
                ...
                if (di.getSignDate() != null) {
                    System.out.println(" - SignDate: " + new SimpleDateFormat().format(di.getSignDate()));
                }
                ...
                System.out.print(printChecks(di.getIndeterminateChecks(), " +++ INDETERMINATE CHECKS +++"));
                    ...
                    if (certificate != null) {
                        ...
                        for (int j = 0; j < certificate.length; j++) {
                            System.out.println("    + Cert[" + j + "] = "
                                    + org.fundaciobit.pluginsib.core.v3.utils.Base64.encode(certificate[j]));
                        }
